...
def serve() -> None:
    """Start the FastAPI server (registered as `aig-server` script)."""
...
def _admin_browser_open_enabled() -> bool:
    """Whether to open /admin when the server becomes reachable (AIG_OPEN_BROWSER)."""

    raw = os.environ.get("AIG_OPEN_BROWSER", "true").strip().lower()
...
def _schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        target=open_when_ready, name="aig-open-admin-browser", daemon=True
    ).start()
...
def init() -> None:
    """Scaffold config at ~/.aig/.env (registered as `aig-init`)."""
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: aig-server"
        )
        return
...
def _migrate_legacy_env_if_missing() -> Path | None:
    ...
    # TODO: Remove after the ~/.aig/.env migration has had a release cycle.

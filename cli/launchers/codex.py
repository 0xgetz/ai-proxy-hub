...
_CODEX_AUTH_ENV_KEY = "AIG_CODEX_API_KEY"
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with AI Gateway proxy configuration."""

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := preflight_proxy(proxy_root_url):
        print(
            f"AI Gateway proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: aig-server", file=sys.stderr)
        raise SystemExit(1)
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral AIG provider config."""
...
def build_codex_launcher_env(
    *,
    ...
    env[_CODEX_AUTH_ENV_KEY] = auth_token.strip() or "aig-no-auth"
    return env
...
def codex_model_catalog_config_args(
    ...
    try:
        ...
        if not isinstance(models, list) or not models:
            print(
                "AI Gateway warning: Codex model catalog is empty; "
        ...
        print(
            "AI Gateway warning: could not prepare Codex model catalog "
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral AIG provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "aig"),
        "-c",
        _toml_assignment("model_providers.aig.name", "AI Gateway"),
        "-c",
        _toml_assignment("model_providers.aig.base_url", _ensure_v1_url(api_url)),
        "-c",
        _toml_assignment("model_providers.aig.env_key", _CODEX_AUTH_ENV_KEY),
        "-c",
        _toml_assignment("model_providers.aig.wire_api", "responses"),
    ]
    if model:

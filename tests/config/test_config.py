...
class TestSettings:
    ...
    def test_default_claude_workspace_uses_aig_home(self, monkeypatch, tmp_path):
        """Unset CLAUDE_WORKSPACE stores agent data under ~/.aig."""
        from config.settings import Settings
        ...
        assert settings.claude_workspace == str(tmp_path / ".aig" / "agent_workspace")

    def test_server_log_path_uses_aig_home(self, monkeypatch, tmp_path):
        """The server log location is fixed under ~/.aig."""
        from config.paths import server_log_path

        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("USERPROFILE", str(tmp_path))

        assert server_log_path() == tmp_path / ".aig" / "logs" / "server.log"
    ...
    def test_blank_claude_workspace_uses_aig_home(self, monkeypatch, tmp_path):
...
class TestPerModelMapping:
    ...
    def test_configured_chat_model_refs_collects_unique_models_with_sources(
        ...
        monkeypatch.setenv("AIG_SMOKE_MODEL_NVIDIA_NIM", "nvidia_nim/smoke")

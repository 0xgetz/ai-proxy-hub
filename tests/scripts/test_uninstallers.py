...
def test_uninstall_sh_removes_uv_tool_and_purges_aig_home() -> None:
    text = _script_text("uninstall.sh")
    tool_body = _braced_body(text, "uninstall_ai_gateway()")
    purge_body = _braced_body(text, "purge_aig_home()")

    assert "Does not remove uv, Claude Code, Codex" in text
    assert "uv tool uninstall" in tool_body
    assert 'PACKAGE_NAME="ai-gateway"' in text
    ...
    assert ".aig" in purge_body
    assert "npm uninstall" not in text
    assert "uv self uninstall" not in text
    assert "uv python uninstall" not in text


def test_uninstall_sh_fails_when_aig_commands_are_running() -> None:
    text = _script_text("uninstall.sh")
    guard_body = _braced_body(text, "assert_no_aig_processes_running()")
...
def test_uninstall_ps1_removes_uv_tool_and_purges_aig_home() -> None:
    ...
    assert "Does not remove uv, Claude Code, Codex" in text
    assert "uv tool uninstall" in tool_body
    assert '$PackageName = "ai-gateway"' in text
    assert "uv not found on PATH; skipping uv tool uninstall." in tool_body
...
def test_uninstall_sh_generic_uv_failure_does_not_delete_aig_home(
    tmp_path: Path,
) -> None:
    sh = shutil.which("sh")
    if sh is None:
    ...
    assert aig_home.exists()
    assert "failed with exit code 42" in result.stderr
    assert "before deleting ~/.aig" in result.stderr


def test_uninstall_sh_missing_tool_still_deletes_aig_home(tmp_path: Path) -> None:
    sh = shutil.which("sh")
    if sh is None:
        pytest.skip("sh is not available on this platform")
    ...
    assert result.returncode == 0
    assert not aig_home.exists()


def test_uninstall_sh_missing_uv_still_deletes_aig_home(tmp_path: Path) -> None:
    sh = shutil.which("sh")
    if sh is None:
        pytest.skip("sh is not available on this platform")
...
def test_uninstall_ps1_generic_uv_failure_does_not_delete_aig_home(
    tmp_path: Path,
) -> None:
...
def test_uninstall_ps1_missing_tool_still_deletes_aig_home(tmp_path: Path) -> None:
    pwsh = shutil.which("pwsh") or shutil.which("powershell")
    if pwsh is None:
        pytest.skip("PowerShell is not available on this platform")
    ...
    _write_mock_uv(
        bin_dir,
        message="tool ai-gateway is not installed",
        exit_code=2,
    )
...
def test_uninstall_ps1_missing_uv_still_deletes_aig_home(tmp_path: Path) -> None:
    pwsh = shutil.which("pwsh") or shutil.which("powershell")
    if pwsh is None:
        pytest.skip("PowerShell is not available on this platform")

"""Tests for cli/entrypoints.py — aig-init scaffolding logic."""
...
def test_init_migrates_home_checkout_env_before_template(tmp_path: Path) -> None:
    """init() preserves users who kept config in ~/ai-gateway/.env."""
    legacy_env = tmp_path / "ai-gateway" / ".env"
...
def test_init_migrates_legacy_xdg_env_before_template(tmp_path: Path) -> None:
    """init() preserves users who kept config in ~/.config/ai-gateway/.env."""
    legacy_env = tmp_path / ".config" / "ai-gateway" / ".env"
    legacy_env.parent.mkdir(parents=True)
...
def test_legacy_env_migration_does_not_overwrite_managed_env(
    ...
    """Legacy migration never overwrites an existing ~/.aig/.env."""
    from cli.entrypoints import _migrate_legacy_env_if_missing

    managed_env = tmp_path / ".aig" / ".env"
...
def test_init_creates_parent_directories(tmp_path: Path) -> None:
    """init() creates ~/.aig/ even if it doesn't exist."""
    config_dir = tmp_path / ".aig"
...
def test_init_prints_next_step_hint(tmp_path: Path) -> None:
    """init() tells the user to run aig-server after editing .env."""
    output, _ = _run_init(tmp_path)

    assert "aig-server" in output
...
def test_cli_scripts_are_registered() -> None:
    ...
    assert scripts["aig-server"] == "cli.entrypoints:serve"
    assert scripts["ai-gateway"] == "cli.entrypoints:serve"
    assert scripts["aig-claude"] == "cli.launchers.claude:launch"
    assert scripts["aig-codex"] == "cli.launchers.codex:launch"
...
def test_schedule_open_admin_browser_opens_when_health_ready(
    ...
    monkeypatch.delenv("AIG_OPEN_BROWSER", raising=False)
...
def test_schedule_open_admin_browser_skips_when_disabled(
    ...
    monkeypatch.setenv("AIG_OPEN_BROWSER", "0")
    from cli import entrypoints
...
def test_serve_migrates_legacy_env_before_loading_settings(tmp_path: Path) -> None:
    ...
    assert (tmp_path / ".aig" / ".env").read_text("utf-8") == (
...
def test_claude_child_env_uses_sentinel_for_blank_configured_auth_token() -> None:
    ...
    assert env["ANTHROPIC_AUTH_TOKEN"] == "aig-no-auth"
    assert "ANTHROPIC_API_KEY" not in env
...
def test_launch_codex_passes_responses_config_and_child_env(
    ...
    assert 'model_provider="aig"' in command
    assert 'model_providers.aig.base_url="http://127.0.0.1:9191/v1"' in command
    assert 'model_providers.aig.wire_api="responses"' in command
    ...
    child_env = popen.call_args.kwargs["env"]
    assert child_env["AIG_CODEX_API_KEY"] == "proxy-token"
...
def test_launch_claude_unreachable_proxy_exits_with_hint(
    ...
    assert "aig-server" in captured.err

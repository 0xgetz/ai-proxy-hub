...
def test_aig_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".aig" / ".env").is_file()


def test_ai_gateway_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
        command=cmd_ai_gateway_serve(),
...
def test_claude_cli_prompt_when_available(
    ...
    with start_server(
        ...
        result = subprocess.run(
            [claude_bin, "-p", "Reply with exactly AIG_SMOKE_PONG"],
            cwd=tmp_path,
            env=env,
            capture_output=True,
            text=True,
    ...
    if "AIG_SMOKE_PONG" not in result.stdout:

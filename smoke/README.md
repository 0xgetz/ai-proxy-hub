...
The second command skips everything unless `AIG_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:AIG_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "providers"
...
catalog/docs and can be overridden with `AIG_SMOKE_MODEL_<PROVIDER>`, for example
`AIG_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`AIG_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `aig-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `AIG_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `AIG_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "ollama"
...
```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "telegram,discord,voice"
$env:AIG_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "nvidia_nim_cli"
$env:AIG_SMOKE_NIM_MODELS = "z-ai/glm-5.1,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "openrouter_free_cli"
$env:AIG_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:AIG_LIVE_SMOKE = "1"
$env:AIG_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `AIG_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `AIG_LIVE_SMOKE=1`: enables live smoke execution.
- `AIG_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `AIG_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `AIG_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `AIG_SMOKE_MODEL_NVIDIA_NIM`, `AIG_SMOKE_MODEL_OPEN_ROUTER`,
  `AIG_SMOKE_MODEL_MISTRAL`, `AIG_SMOKE_MODEL_MISTRAL_CODESTRAL`,
  `AIG_SMOKE_MODEL_DEEPSEEK`, `AIG_SMOKE_MODEL_KIMI`,
  `AIG_SMOKE_MODEL_WAFER`, `AIG_SMOKE_MODEL_OPENCODE`, `AIG_SMOKE_MODEL_OPENCODE_GO`,
  `AIG_SMOKE_MODEL_ZAI`, `AIG_SMOKE_MODEL_FIREWORKS`,   `AIG_SMOKE_MODEL_GEMINI`,
  `AIG_SMOKE_MODEL_GROQ`, `AIG_SMOKE_MODEL_CEREBRAS`,
  `AIG_SMOKE_MODEL_LMSTUDIO`,
  `AIG_SMOKE_MODEL_LLAMACPP`, `AIG_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `AIG_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `AIG_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `AIG_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `AIG_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `AIG_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `AIG_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `AIG_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `AIG_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `AIG_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `AIG_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
`uv run`, so Windows does not try to replace `ai-gateway.exe` while it is
locked.
...
- `target_disabled`: skipped because `AIG_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `AIG_SMOKE_PROVIDER_MATRIX`;

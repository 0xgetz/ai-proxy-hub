...
TARGET_REQUIRED_ENV: dict[str, tuple[str, ...]] = {
    "api": (),
    "auth": (),
    "cli": ("AIG_SMOKE_CLAUDE_BIN", "configured provider for Claude CLI prompt"),
    "clients": (),
    "config": (),
    "extensibility": (),
    "messaging": (),
    "providers": ("configured provider credentials/endpoints or AIG_SMOKE_MODEL_*",),
    ...
    "nvidia_nim_cli": (
        "NVIDIA_NIM_API_KEY",
        "AIG_SMOKE_CLAUDE_BIN or claude on PATH",
    ),
    "openrouter_free_cli": (
        "OPENROUTER_API_KEY",
        "AIG_SMOKE_CLAUDE_BIN or claude on PATH",
    ...
    "telegram": (
        "TELEGRAM_BOT_TOKEN",
        "ALLOWED_TELEGRAM_USER_ID or AIG_SMOKE_TELEGRAM_CHAT_ID",
    ),
    "discord": (
        "DISCORD_BOT_TOKEN",
        "ALLOWED_DISCORD_CHANNELS or AIG_SMOKE_DISCORD_CHANNEL_ID",
    ),
    "voice": ("VOICE_NOTE_ENABLED=true", "AIG_SMOKE_RUN_VOICE=1"),
}
...
class SmokeConfig:
    ...
    def load(cls) -> SmokeConfig:
        ...
        return cls(
            ...
            live=os.getenv("AIG_LIVE_SMOKE") == "1",
            interactive=os.getenv("AIG_SMOKE_INTERACTIVE") == "1",
            targets=_parse_targets(os.getenv("AIG_SMOKE_TARGETS")),
            provider_matrix=_parse_csv(os.getenv("AIG_SMOKE_PROVIDER_MATRIX")),
            timeout_s=float(os.getenv("AIG_SMOKE_TIMEOUT_S", "45")),
            prompt=os.getenv("AIG_SMOKE_PROMPT", "Reply with exactly: AIG_SMOKE_PONG"),
            claude_bin=os.getenv("AIG_SMOKE_CLAUDE_BIN", "claude"),
    ...
    def _include_provider_in_smoke(
        ...
        if self.provider_matrix and provider in self.provider_matrix:
            return True
        return bool(os.getenv(f"AIG_SMOKE_MODEL_{provider.upper()}"))
...
def _provider_smoke_model(provider: str) -> tuple[str, str]:
    override_env = f"AIG_SMOKE_MODEL_{provider.upper()}"
...
def _normalize_provider_model(provider: str, raw_model: str) -> str:
    ...
    if not model:
        msg = f"AIG_SMOKE_MODEL_{provider.upper()} must not be empty"
    ...
    if prefix in SUPPORTED_PROVIDER_IDS:
        msg = (
            f"AIG_SMOKE_MODEL_{provider.upper()} must use provider prefix "
...
def nvidia_nim_cli_model_refs(
    ...
    explicit_models = _parse_csv_ordered(source.get("AIG_SMOKE_NIM_MODELS"))
    extra_models = _parse_csv_ordered(source.get("AIG_SMOKE_NIM_EXTRA_MODELS"))

    if "AIG_SMOKE_NIM_MODELS" in source and not explicit_models:
        raise ValueError("AIG_SMOKE_NIM_MODELS must list at least one model")

    models: list[tuple[str, str]] = []
    base_models = explicit_models or NVIDIA_NIM_CLI_DEFAULT_MODELS
    base_source = (
        "AIG_SMOKE_NIM_MODELS" if explicit_models else "nvidia_nim_cli_default"
    )
    models.extend((model, base_source) for model in base_models)
    models.extend((model, "AIG_SMOKE_NIM_EXTRA_MODELS") for model in extra_models)
...
def openrouter_free_cli_model_refs(
    ...
    source = env if env is not None else os.environ
    explicit_models = _parse_csv_ordered(source.get("AIG_SMOKE_OPENROUTER_FREE_MODELS"))
    extra_models = _parse_csv_ordered(
        source.get("AIG_SMOKE_OPENROUTER_FREE_EXTRA_MODELS")
    )

    if "AIG_SMOKE_OPENROUTER_FREE_MODELS" in source and not explicit_models:
        raise ValueError(
            "AIG_SMOKE_OPENROUTER_FREE_MODELS must list at least one model"
        )

    models: list[tuple[str, str]] = []
    base_models = explicit_models or OPENROUTER_FREE_CLI_DEFAULT_MODELS
    base_source = (
        "AIG_SMOKE_OPENROUTER_FREE_MODELS"
        if explicit_models
        else "openrouter_free_cli_default"
    )
    models.extend((model, base_source) for model in base_models)
    models.extend(
        (model, "AIG_SMOKE_OPENROUTER_FREE_EXTRA_MODELS") for model in extra_models
    )

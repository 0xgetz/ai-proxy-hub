...
This document is a maintainer-oriented map of AI Gateway. It explains the
...
AI Gateway is a local proxy for agent clients. It accepts Anthropic
...
AIG optimizes for installed user workflows, not internal compatibility. The
...
- `aig-server` and the local Admin UI for configuring supported providers,
  model routing, auth, server tools, messaging, and diagnostics.
- `aig-claude`, Claude Code, and the Anthropic-compatible proxy behavior Claude
...
- `aig-codex`, Codex CLI/extensions, and the streaming OpenAI Responses behavior
...
- `aig-server` and `ai-gateway` call `cli.entrypoints:serve`.
- `aig-init` calls `cli.entrypoints:init`.
- `aig-claude` calls `cli.launchers.claude:launch`.
- `aig-codex` calls `cli.launchers.codex:launch`.
...
and [scripts/uninstall.ps1](scripts/uninstall.ps1) remove only the AIG uv tool and always
delete the managed `~/.aig/` tree from [config/paths.py](config/paths.py); they do not remove
...
1. repo-local `.env`;
2. managed `~/.aig/.env`;
3. optional `AIG_ENV_FILE`, appended when present.
...
- config directory: `~/.aig`;
- managed env file: `~/.aig/.env`;
- generated Codex model catalog: `~/.aig/codex-model-catalog.json`;
- agent workspace: `~/.aig/agent_workspace`;
- server log: `~/.aig/logs/server.log`.
...
Codex-specific model picker shaping stays out of this route. `aig-codex` fetches
the same `/v1/models` response at launch, converts AIG gateway IDs into
provider-selectable Codex slugs, writes `~/.aig/codex-model-catalog.json`, and
passes it as `model_catalog_json`. Codex users open the native picker with
`/model`; AIG does not implement a proxy-level `/models` alias.
...
- streaming-only `/v1/responses` support for Codex/AIG workflows;
...
AIG accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed AIG/Codex workflows only need
...
AIG does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and AIG
...
- `aig-claude` strips inherited `ANTHROPIC_*` variables, sets
  ...
  becomes the local-only `aig-no-auth` sentinel so Claude Code reaches the proxy
  instead of stopping at its login gate.

[cli/launchers/codex.py](cli/launchers/codex.py) owns the installed
`aig-codex` launcher:

- `aig-codex` strips official OpenAI and Codex credential variables.
- It creates an ephemeral `aig` model provider with `wire_api = "responses"` and
  a base URL pointing at the local proxy `/v1` path.
- After proxy health succeeds, it fetches `/v1/models`, writes a generated Codex
  `model_catalog_json` file under `~/.aig/`, and injects that path so Codex's
  native `/model` picker lists AIG provider slugs. Catalog generation is
  fail-open: launch continues with a warning if the catalog cannot be prepared.
- It stores the proxy auth token in `AIG_CODEX_API_KEY` for Codex to read.
...
Codex is supported through `aig-codex` and Codex extensions. AIG does not keep an
